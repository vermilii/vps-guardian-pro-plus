import os, time, json, yaml, threading, requests, psutil
...
OFFSET_FILE = "/opt/vps-guardian/.tg_offset"   # persist offset agar tidak replay update lama
...
class SimpleBot:
    ...
    def answer_callback(self, cb_id: str, text: str):
        ...
        try:
            requests.post(url, data={"callback_query_id": cb_id, "text": text, "show_alert": False}, timeout=10)
        except Exception:
            pass

    def on_callback(self, prefix: str, handler):
        self.cb_handlers.append((prefix, handler))

    def on_command(self, text: str, handler):
        self.cmd_handlers[text] = handler
            ...
            try:
                ...
                for upd in r.json().get("result", []):
                    ...
                    if "callback_query" in upd:
                        ...
                        data = cb.get("data", "")
                        if cb.get("message", {}).get("chat", {}).get("id") != int(self.chat_id):
                            continue
                        for pref, handler in self.cb_handlers:
                            if data.startswith(pref):
                                try:
                                    handler(cb.get("id"), {"data": data})
                                except Exception:
                                    pass
                        continue
                    ...
                    # tombol menu (keyboard) memetakan ke perintah slash
                    mapping = {
                        "📊 Status": "/status",
                        "🧪 Self-test": "/selftest",
                        "🚫 Blocked": "/blocked",
                    ...
                    if text in mapping and mapping[text] in self.cmd_handlers:
                        try:
                            self.cmd_handlers[mapping[text]]()
                        except Exception:
                            pass
                        continue
            except Exception:
...
def main():
    ...
    def notify(msg: str, buttons=None):
        bot.send_message(f"<b>[VPS Guardian]</b> {msg}", buttons=buttons)

    MENU_ROWS = [
        ["📊 Status", "🧪 Self-test"],
        ["🚫 Blocked", "🔁 Restart Agent"],
        ["🛡 Fail2ban", "☁️ Cloudflare"],
        ["📋 Menu"],
    ]
    ...
    def cmd_restart_agent():
        ...
        # delay 1s agar offset tersimpan & ack dulu sebelum proses mati
        os.system('nohup bash -c "sleep 1; systemctl restart vps-guardian" >/dev/null 2>&1 &')

    def cmd_blocked():
        st = StateStore(cfg["state"]["file"])
        ips = list(st.state.get("blocked_ips", {}).keys())
        notify("Blocked IPs: " + (", ".join(ips) if ips else "-"))

    def cmd_f2b_status():
        if not f2b_available():
            notify("Fail2ban not installed/available.")
            return
        notify("<b>Fail2ban</b>\n<pre>" + f2b_status() + "</pre>")

    def cmd_cf_status():
        cfc = cfg.get("cloudflare", {})
        if not cfc.get("enable", False):
            notify("Cloudflare integration disabled.")
            return
        cf = CF(cfc.get("api_token", ""), cfc.get("zone_id", ""))
        notify(cf.status())
    ...
    for name, func in {
        ...
        "/restart_agent": cmd_restart_agent,
        "/blocked": cmd_blocked,
        "/f2b_status": cmd_f2b_status,
        "/cf_status": cmd_cf_status,
    }.items():
        bot.on_command(name, func)

    # juga daftarkan label tombol (tanpa harus ketik /command)
    bot.on_command("📋 Menu", cmd_menu)
    bot.on_command("📊 Status", cmd_status)
    bot.on_command("🧪 Self-test", cmd_selftest)
    bot.on_command("🚫 Blocked", cmd_blocked)
    bot.on_command("🔁 Restart Agent", cmd_restart_agent)
    bot.on_command("🛡 Fail2ban", cmd_f2b_status)
    bot.on_command("☁️ Cloudflare", cmd_cf_status)

    # Inline callback: quick actions restart <svc>
    def on_qa_restart(cb_id, payload):
        data = payload["data"]
        try:
            _, _, svc = data.split(":", 2)
        except Exception:
            bot.answer_callback(cb_id, "Bad data")
            return
        bot.answer_callback(cb_id, f"Restart {svc} dikirim")
        os.system(f'nohup bash -c "sleep 1; systemctl restart {svc}" >/dev/null 2>&1 &')
        notify(f"🔁 Quick Action: restart {svc} dipicu dari Telegram.")

    bot.on_callback("qa:restart:", on_qa_restart)

    # mulai polling & worker threads
    bot.start_polling()
